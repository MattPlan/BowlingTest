...
//		game.addFrame(new LastFrame(10));	
	 ...
	 * The number of the roll is either 1 or 2 and the score is between 0 and 10 for a normalFrame.
	 */
	...
	/**
	 * Test that declaring two times a score for the same roll of a normalFrame is throwing a 
	 * BowlingException.
	 */
	@Test
	...
	/**
	 * Test that declaring a third roll for a normalFrame is throwing a BowlingException.
	 */
	@Test
	...
	//------------------------------NormalFrame.getPinsDown(int roll)------------------------------
	
	/**
	 * This method getPinsDown(int roll) should return the number of pins down for a particular roll
	 * of a given frame. Throws a BowlingException if the roll number is not valid.
	 */
	
	/**
	 * Test that NormalFrame.getPinsDown(int roll) returns the correct number of pins down after 
	 * setting a score for the first roll.
	 */
	@Test
	...
	/**
	 * Test that NormalFrame.getPinsDown(int roll) returns the correct number of pins down after 
	 * setting a score for the two rolls.
	 */
	@Test
	...
	/**
	 * Test that NormalFrame.getPinsDown(int roll) throws a BowlingException if the roll number 
	 * is invalid.
	...
	public void testNormalFrame_getPinsDown_invalidRollNumber(int x) {
		assertThrows("Invalid roll number : ", BowlingException.class,
				() -> normalFrame.getPinsDown(x));
	}
	
	/**
	 * Test that NormalFrame.getPinsDown(int roll) throw a BowlingException after a call to the
	 * reset() method as the score doesn't exist yet.
	 */
	@Test
	...
	/**
	 * Test that for a NormalFrame the toString() method returns a string made of two characters.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.getFrameNumber() returns the correct number equals to the frame number.
	 */
	@Test
	...
	//---------------------------LastFrame.setPinsDown(int roll, int score)------------------------
	
	/**
	 * There are two rolls per frame. The method setPinsDown(int roll, int score) is setting the
	 * number of pins down (the score) for a given roll in the frame.
	 * The number of the roll is either 1 or 2 and the score is between 0 and 10 for a normalFrame.
	 * As it is the last frame of the game, there can be 3 rolls and the score can be higher than 10.
	 */
	
	/**
	 * Test that a value like 1 or 2 for the roll's number in LastFrame.setPinsDown and a number for 
	 * the score inside the range 0-10 does not throw a BowlingException.
	 * @param x  the first roll number for the frame
	 * 		  y  the number of pins down (the score) for the first roll
	 * 		  a  the second roll number 
	...
	public void testLastFrame_setPinsDown_validParameters(int x, int y, int a, int b) {
		new LastFrame(10).setPinsDown(x, y).setPinsDown(a, b);
	}
	
	/**
	 * Test that a value other than 1 or 2 for the roll's number in LastFrame.setPinsDown throws
	 * a BowlingException.
	 * Plus, also test that number outside the range 0-9 for the score throws a BowlingException.
	 * @param x  the roll number for the frame
	 * 		  y  the number of pins down (the score)
	...
	public void testLastFrame_setPinsDown_wrongParameters(int x, int y) {
		assertThrows("Out of bound parameters : ", BowlingException.class, 
				() -> new LastFrame(10).setPinsDown(x, y));
	}
	
	/**
	 * Test that we can declare a 3rd roll for a lastFrame if the score of the two first rolls is
	 * equal or higher than 10.
	 * Testing several configurations : strike/gutter, strike/score, strike/spare, spare/score.
	 * @param x  the first roll number for the frame
	 * 		  y  the number of pins down (the score) for the first roll
	 * 		  a  the second roll number 
	 * 		  b  the score for the second roll
	 * 		  r  the third roll number
	 * 		  s  the score for the third roll
	 */
	@ParameterizedTest
	...
	public void testLastFrame_setPinsDown_validParametersFor3rdRoll(int x, int y, int a, int b, int r, int s) {
		new LastFrame(10).setPinsDown(x, y).setPinsDown(a, b).setPinsDown(r, s);
	}
	
	/**
	 * Test that we can't declare a 3rd roll for a lastFrame if the score of the two first rolls is
	 * lower than 10. Throws a BowlingException.
	 * @param x  the first roll number for the frame
	 * 		  y  the number of pins down (the score) for the first roll
	 * 		  a  the second roll number 
	 * 		  b  the score for the second roll
	...
	/**
	 * Test that declaring two times a score for the same roll of a LastFrame is throwing a 
	 * BowlingException.
	 */
	@Test
	...
	/**
	 * Test that we can't declare a score for the 2nd roll before having declared one for the 1st
	...
	/**
	 * Test that we can't declare a score for the 3rd roll before having declared a total score equal
	 * or higher than 10 in the two first rolls. Throws a BowlingException.
	 */
	@Test
	...
	/**
	 * Test that declaring a score higher than 10 with the sum of the two rolls' score for a frame is 
	 * throwing a BowlingException.
	 */
	@Test
	...
	/**
	 * There are two rolls and sometimes three for a LastFrame. The method countPinsDown() returns 
	 * the score of the frame between 0 and 30. After a call to the method setPinsDown(int roll, int score) 
	 * this method returns the sum of the score of the two or three rolls.
	 */
	...
	public void testLastFrame_countPinsDown_afterInitialization() {
		assertEquals("Wrong number of pins down after initialization : ", 0, 
				new LastFrame(10).countPinsDown());
	}
	...
	/**
	 * Test that after setting a score for the two rolls of a frame allowing a third roll, countPinsDown 
	...
	/**
	 * There are two or three rolls for a LastFrame. The method countRolls() is returning an integer 
	 * between 0 and 3 depending on the number of roll the player already made for this frame.
	 * After a reset of the frame, it should return 0.
	 */
	...
	/**
	 * Test that LastFrame.countRolls() returns 1 when a score between 1 and 9 has been set for
	 * the first roll.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.countRolls() returns 2 when a score between 1 and 9 has been set for
	 * the two rolls of the frame. The sum of the two score doesn't exceed 10.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.countRolls() returns 3 when the score of the two first frame is equal
	 * or higher than 10 unlocking the third roll.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.countRolls() returns 1 when a score of 0 has been set for the first 
	 * roll. That is when the player made a gutter.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.countRolls() returns 2 when a score of 0 has been set for the two 
	 * rolls of a frame. That is when the player made two gutters.
	 */
	@Test
	...
	//-------------------------------LastFrame.getPinsDown(int roll)-------------------------------
	
	/**
	 * This method getPinsDown(int roll) should return the number of pins down for a particular roll
	 * of a given frame. Throws a BowlingException if the roll number is not valid.
	 * A LastFrame can have three rolls.
	 */
	
	/**
	 * Test that LastFrame.getPinsDown(int roll) returns the correct number of pins down after 
	 * setting a score for the first roll.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.getPinsDown(int roll) returns the correct number of pins down after 
	 * setting a score for the two rolls enabling the third roll.
	 */
	@Test
	...
	/**
	 * Test that LastFrame.getPinsDown(int roll) throws a BowlingException if the roll number 
	 * is invalid.
	 * @param x  the roll number
	 */
	@ParameterizedTest
	...
	public void testLastFrame_getPinsDown_invalidRollNumber(int x) {
		assertThrows("Invalid roll number : ", BowlingException.class,
				() -> lastFrame.getPinsDown(x));
	}
	
	/**
	 * Test that LastFrame.getPinsDown(int roll) throw a BowlingException after a call to the
	 * reset() method as the score doesn't exist yet.
	 */
	@Test
	...
	/**
	 * Test that for a LastFrame the toString() method returns a string made of three characters.
	 */
	@Test
	public void testLastFrame_toString() {
		...
		assertEquals("Wrong number of characters with a strike for LastFrame", 3, 
				new LastFrame(10).setPinsDown(1, 10).setPinsDown(2, 0).setPinsDown(3, 10).toString().length());
		assertEquals("Wrong number of characters with a spare for LastFrame", 3, 
